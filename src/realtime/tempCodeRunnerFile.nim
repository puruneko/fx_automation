...
proc maDoubleIndicator(inds: Indicators, itr: int, args: IndicatorArgument): float =
  return inds[args["targetIndicator"]].maDouble(itr, parseInt(args["period"]))
...
proc rangeCorrelation(inds: Indicators, itr: int, args: IndicatorArgument): float =
  ...
  let rangeWavelet = inds[args["targetIndicator"]][itr-period+1..itr].limitedSequence(sgmCoef)
  return inds[args["targetIndicator"]][itr-period+1..itr].correlation(rangeWavelet)
...
proc trendCorrelation(inds: Indicators, itr: int, args: IndicatorArgument): float =
  ...
  if itr < period:
    return NaN
  let sgmCoef = parseFloat(args["sgmCoef"])
  let rLen = inds[args["targetIndicator"]][itr-period+1..itr].limitedSequence(sgmCoef).len
  let maxVal = max(inds[args["targetIndicator"]][itr-period+1..itr])#max(inds[args["targetIndicator"]][itr-rLen+1..itr])
  let minVal = min(inds[args["targetIndicator"]][itr-period+1..itr])#min(inds[args["targetIndicator"]][itr-rLen+1..itr])
  let coef = (maxVal-minVal)/float(rLen)
  let trendWavelet = lc[minVal + float(x)*coef | (x <- 0..<rLen), float]
  return inds[args["targetIndicator"]][itr-period+1..itr].correlation(trendWavelet)
...
proc sampleLong_take(inds: Indicators, itr: int): DirectionType =
  when tradeFalg:
    if numericCross(inds["trendCorrelationM_ma"], itr) == 1:
...
proc sampleLong_release(inds: Indicators, itr: int, takeRecord: TradeRecord): DirectionType =
  when tradeFalg:
    ...
    if numericCross(inds["trendCorrelationS_ma"], itr) == -1:
      return reverseDirection(takeRecord.direction)
  return DirectionType.nop
...
proc main(pt: var PhaseTimer) =
  ...
  pt.phase("settings"):
    ...
    pt.phase("indicator settings"):
      ...
      #
      #inds.push("rangeCorrelation", rangeCorrelation, &&{"targetIndicator": "mam", "period": $mamPeriod, "sgmCoef": "1.0"}, &&{"display": "", "position": "0", "oppositeAxis": "true"})
      inds.push("trendCorrelationS", trendCorrelation, &&{"targetIndicator": "map", "period": $malPeriod, "sgmCoef": "0.25"}, &&{"display": "", "position": "0", "oppositeAxis": "true"})
      inds.push("trendCorrelationM", trendCorrelation, &&{"targetIndicator": "map", "period": $malPeriod, "sgmCoef": "0.5"}, &&{"display": "", "position": "0", "oppositeAxis": "true"})
      #inds.push("rangeCorrelation_ma", maIndicator, &&{"targetIndicator": "rangeCorrelation", "period": $masPeriod}, &&{"display": $true, "position": "0", "oppositeAxis": "true"})
      inds.push("trendCorrelationS_ma", maIndicator, &&{"targetIndicator": "trendCorrelationS", "period": $masPeriod}, &&{"display": $true, "position": "0", "oppositeAxis": "true"})
      inds.push("trendCorrelationM_ma", maIndicator, &&{"targetIndicator": "trendCorrelationM", "period": $masPeriod}, &&{"display": $true, "position": "0", "oppositeAxis": "true"})
